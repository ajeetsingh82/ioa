...
from .model.models import (
    ...
    ArchitectResponse # Still needed for final step
)
...
class ConductorAgent(Agent):
    def __init__(self, name: str, seed: str):
        ...
        self.on_message(model=ArchitectResponse)(self.handle_architect_response) # Keep for now
    ...
    async def trigger_architect(self, ctx: Context, pipeline):
        ...
        if architect_addr:
            # Architect still uses its own model for now, can be refactored later
            from .model.models import ArchitectRequest
            await ctx.send(
                architect_addr,
                ArchitectRequest(
                    request_id=pipeline.request_id,
